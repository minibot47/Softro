...
export default function Process() {
  return (
    <section className="py-16 md:py-24 px-4 sm:px-6 mt-12 md:mt-20 mb-12 md:mb-20 bg-[url(/images/banner-bg.png)] dark:bg-none dark:bg-[#151a16] bg-contain bg-top rounded-[24px] border border-green-500/30 dark:border-gray-700">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-8 mb-10 md:mb-14">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 sm:gap-5 w-full md:w-[55%]">
            <span className="text-base font-normal text-black dark:text-white border border-green-500 dark:border-green-700 px-2 py-0.5 rounded-full shrink-0 w-fit">Process</span>
        ...
        <div className="relative flex flex-col lg:flex-row items-center justify-center gap-8 lg:gap-2 mt-12 lg:mt-32">

        {/* Card 1 */}
        <div
          className="rounded-2xl p-8 relative overflow-hidden group transition-all duration-300 hover:-translate-y-1 w-full max-w-[300px] lg:w-[300px] min-h-[260px] lg:h-[295px] lg:flex-shrink-0"
        ...
        >
          ...
          <h3 className="font-bold text-xl mb-3 text-black dark:text-black">{steps[0].title}</h3>
          <p className="text-gray-600 text-sm leading-relaxed dark:text-black">{steps[0].desc}</p>
        </div>

        {/* Connector 1 */}
        <div className="hidden lg:block" style={{ width: '181px', height: '124px', flexShrink: 0 }}>
          <img src="/images/creativeprocess1.svg" alt="" className="w-full h-full mt-5"/>
        </div>

        {/* Card 2 — middle */}
        <div
          className="rounded-2xl p-8 relative overflow-hidden group transition-all duration-300 hover:-translate-y-1 lg:-mt-20 w-full max-w-[300px] lg:w-[300px] min-h-[260px] lg:h-[295px] lg:flex-shrink-0"
        ...
        >
          ...
          <h3 className="font-bold text-xl mb-3 text-black dark:text-black">{steps[1].title}</h3>
          <p className="text-gray-600 text-sm leading-relaxed dark:text-black">{steps[1].desc}</p>
        </div>
        ...
        >
          ...
          <h3 className="font-bold text-xl mb-3 text-black dark:text-black">{steps[2].title}</h3>
          <p className="text-gray-600 text-sm leading-relaxed dark:text-black">{steps[2].desc}</p>
        </div>
