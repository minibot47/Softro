...
export default function Solutions() {
  return (
    <section className="py-24 px-4 sm:px-8 md:px-16 bg-white max-w-[1440px] m-auto">

      {/* Header */}
      <div className="flex flex-col sm:flex-row gap-4 sm:gap-[134px] mb-16">
        <div className="flex-shrink-0 sm:mt-1">
          <span className="px-4 py-1.5 rounded-full border border-gray-400 text-sm text-gray-700">
            Roadmap
          </span>
        </div>
        <h2 className="text-3xl sm:text-4xl md:text-5xl font-medium leading-tight tracking-tight text-gray-900 max-w-lg">
      ...
      <div className="relative flex items-center justify-center">
        ...
        <span className="relative bg-[#F8FDF4] px-3 dark:text-black py-1.5 rounded-full border-[0.1px] border-gray-400/40 text-BASE font-semibold text-gray-800 z-10 text-center text-sm sm:text-base">
            ...
            {[...Array(4)].flatMap((_, setIndex) =>
              [1, 2, 3, 4, 5].map((n) => (
                <img
                  ...
                  alt="partner"
                  className="h-7 sm:h-10 flex-shrink-0"
                />
              ))
            )}
          </div>
